 ...
 */
class ItemProxy {
  constructor(model, unselectable) {
    this.id = model.id;
    this._model = model;
    this.pairedWith = [];
...
class Lookup {
  ...
  removeUnselectables(){
    this.list = this.list.filter((proxy)=>!proxy.unselectable);
    return this;
  }
}
...
function select(payload) {
  ...
  comparisons.forEach((c) => {
    ...
    if(!a){
      a = new ItemProxy({id: c.a}, true);
      proxies.add(a);
    }
    if(!b){
      b = new ItemProxy({id: c.a}, true);
      proxies.add(b);
    }
    a.pairWith(b);
    b.pairWith(a);
